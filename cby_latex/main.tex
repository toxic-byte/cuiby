...
% \documentclass[type=master,fontset=siyuan]{hitszthesis}
...
% algorithm 包已在 hitszthesis.sty 中通过 algorithm2e 加载，不要重复加载 algorithm 包
% 以下包已在 hitszthesis.cls 或 hitszthesis.sty 中加载：
%   booktabs, amsmath, multirow, graphicx, hyperref
\usepackage{caption}  % 更灵活的标题支持
\captionsetup{labelsep=space, textformat=simple} % 图表标题：空格分隔，不自动添加末尾句号
\usepackage{pifont}
