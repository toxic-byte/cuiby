...
由于蛋白质功能预测具有多标签、类别不平衡和零样本泛化等特点，其评价指标不能简单沿用普通分类任务中的准确率，当前研究主要遵循CAFA（Critical Assessment of Function Annotation）挑战赛\cite{zhou2019cafa}的蛋白质中心评价协议。对于任意给定阈值$t \in [0,1]$，设模型为每个蛋白质输出一组GO术语预测概率，经阈值化后得到预测标签集合$\hat{Y}_i(t)$，真实标签集合记为$Y_i$，平均精确率和平均召回率分别如公式~\eqref{eq:precision}和式~\eqref{eq:recall}所示：
\begin{equation}
\label{eq:precision}
...
形式化地，设已见类别集合为$\mathcal{Y}_s$，未见类别集合为$\mathcal{Y}_u$，且$\mathcal{Y}_s \cap \mathcal{Y}_u = \varnothing$。每个类别$y$对应一个语义描述向量$\mathbf{a}_y \in \mathbb{R}^{d_a}$。零样本学习的目标是在仅使用$\mathcal{Y}_s$类别的标注样本训练后，对$\mathcal{Y}_u$类别的样本正确分类。其关键在于学习从特征空间到语义空间的映射函数，如公式~\eqref{eq:zsl_mapping}所示：
\begin{equation}
\label{eq:zsl_mapping}
f: \mathcal{X} \to \mathcal{A}
\end{equation}
在推理阶段，给定测试样本$\mathbf{x}$，通过将其映射到语义空间后与各类别的语义表示进行最近邻匹配完成分类，如公式~\eqref{eq:zsl_inference}所示：
\begin{equation}
