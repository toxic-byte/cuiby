...
本章的组织结构如下：2.2节介绍蛋白质功能预测问题的定义与常用评价指标；2.3节介绍零样本学习的基本框架与核心方法；2.4节系统介绍预训练技术的基本原理，涵盖语言模型预训练、蛋白质语言模型、多模态预训练、参数高效微调与知识蒸馏等关键方向；2.5节对本章内容进行总结。
...
零样本学习（Zero-Shot Learning, ZSL）是解决标签空间开放性问题的重要技术方向，也是本论文第三章方法设计的理论基础。本节介绍零样本学习的基本框架、核心方法与关键技术原理，为后续章节的模型设计提供方法论基础。
...
围绕上述框架，零样本学习领域发展出了多类核心方法。早期经典方法主要包括属性空间映射和嵌入对齐两条路线。DAP（Direct Attribute Prediction）\cite{lampert2014attribute}为每个类别人工定义一组语义属性，并通过训练属性分类器组合预测结果来完成推理。DeViSE\cite{frome2013devise}利用预训练视觉特征和词向量，通过排序损失学习跨模态映射；SAE（Semantic Autoencoder）\cite{kodirov2017semantic}则通过编码器—解码器结构联合建模映射与重建约束，以缓解信息丢失问题。然而，嵌入对齐方法普遍面临域偏移（Domain Shift）问题，即训练阶段仅使用已见类别数据导致所学映射在未见类别上不够准确。为此，f-CLSWGAN\cite{xian2018feature}和CADA-VAE\cite{schonfeld2019generalized}等生成式方法利用条件生成模型合成未见类别特征，从根本上缓解训练与测试类别分布不一致的问题。

近年来，基于大规模预训练的零样本范式从根本上改变了技术路径。CLIP\cite{radford2021learning}通过在大规模图像—文本对上进行对比学习预训练，仅需输入类别名称或文本描述便可直接完成零样本分类，无需目标域的训练样本。在此基础上，CoOp\cite{zhou2022learning}和CoCoOp\cite{zhou2022conditional}等提示学习方法通过将提示中的上下文词替换为可学习向量或引入条件提示机制，进一步提升了零样本泛化能力。在语义描述的选择上，零样本学习经历了从人工属性到词向量（如Word2Vec\cite{mikolov2013efficient}、GloVe\cite{pennington2014glove}）、再到预训练语言模型文本嵌入（如BERT\cite{devlin2019bert}）的演变，表明语义空间的表达质量是制约零样本学习性能的关键因素。

上述零样本学习的基本框架和核心技术构成了本论文第三章方法设计的理论基础。蛋白质功能预测天然具备零样本学习的特征——GO标签空间包含数万个功能术语，其中大量术语标注样本极度稀缺甚至完全缺失，且新术语仍在持续创建。GO术语具有层级结构化的本体关系和丰富的文本定义，为构建高质量语义空间提供了独特优势。本论文第三章正是借鉴嵌入对齐和多模态对比学习的思想，将蛋白质功能预测重构为蛋白质特征与GO功能语义之间的匹配问题，以实现对未见功能标签的零样本推断。
...
随后，介绍了零样本学习的基本框架与核心方法，涵盖属性空间映射、嵌入对齐、生成式方法以及基于视觉-语言基础模型的提示学习等关键技术原理，并阐述了零样本学习框架与蛋白质功能预测问题的内在契合性。之后，系统介绍了预训练技术的基本原理，涵盖语言模型预训练、蛋白质语言模型、多模态预训练以及参数高效微调与知识蒸馏等关键方向。
